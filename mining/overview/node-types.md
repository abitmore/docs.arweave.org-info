---
description: Understanding the different Arweave node types
---
...
See the [Hardware Guide](../setup/hardware.md) for guidance on the hardware requirements for each node type, and [Getting Started](../../) for next steps in getting your miner setup.

## 1. Solo Miner

A solo miner is a single node that stores and mines all the data associated with a single mining address. It is in contrast to a [Coordinated Miner](node-types.md#2-coordinated-miner) which involves multiple nodes coordinating to mine all the data associated with a single mining address.
...
* You want full control over keys and payouts (no pool/coordinator in the middle) (see [Pool Miner](node-types.md#23-pool-miner) for more information)
* You can run or rent access to a VDF server (see [VDF Server](node-types.md#24-vdf-server) for more information)
...
For more information see [Coordinated Mining](coordinated-mining.md).
...
For more information see [=Pool Mining](pool-mining.md).
...
For more information see [VDF](vdf.md).
